        ...
        if index < len(subList) - 6:
            ...
            if rule611 and rule629:
                if rule625:
                    ...
                    faultList.append(subList[index][kcommon.stdate])

    if len(dateList) + len(faultList) < 1:
        return -1
    per = len(dateList)/float(len(dateList)+len(faultList))

    # print "---------------" + code + " success4  " + str(len(dateList)) + "down" + str(len(faultList))
    print "---------------" + code + " success4  rase 概率" + str(per)

    print "raise " + str(dateList)
    print "down " + str(faultList)
    return per
